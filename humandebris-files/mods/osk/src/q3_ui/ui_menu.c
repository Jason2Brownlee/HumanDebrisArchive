...
// Zygote - start
	y += MAIN_MENU_VERTICAL_SPACING;
	s_main.osk.generic.type				= MTYPE_PTEXT;
	s_main.osk.generic.flags			= QMF_CENTER_JUSTIFY|QMF_PULSEIFFOCUS;
	s_main.osk.generic.x				= 320;
	s_main.osk.generic.y				= y;
	s_main.osk.generic.id				= ID_OSK;
	s_main.osk.generic.callback			= Main_MenuEvent; 
	s_main.osk.string					= "OSK MENU";
	s_main.osk.color					= color_red;
	s_main.osk.style					= style;
// Zygote - end
	...
	// Zygote - osk arena 
	Menu_AddItem( &s_main.menu,	&s_main.osk );

...
/*	Zygote - must comment out this code to stop powerups spawning in.
...
void ClearRegisteredItems( void ) {
	...
	// Zygote - Register different weapons based on the type of instagib
	switch (g_oskmode.integer)
	{
	case 1:
		RegisterItem( BG_FindItemForWeapon( WP_RAILGUN ) );		// killer rail
		break;
	case 2:
		RegisterItem( BG_FindItemForWeapon( WP_MACHINEGUN ) );	// rifle
		break;
	case 3:
		RegisterItem( BG_FindItemForWeapon( WP_PLASMAGUN ) );	// bounce
		break;
	case 4:
		RegisterItem( BG_FindItemForWeapon( WP_RAILGUN ) );		// telefrag
		break;
	case 5:
		RegisterItem( BG_FindItemForWeapon( WP_GAUNTLET ) );	// gauntlet
		break;
	case 6:
		RegisterItem( BG_FindItemForWeapon( WP_BFG ) );			// delayed
		break;
	case 7:
		RegisterItem( BG_FindItemForWeapon( WP_BFG ) );			// delayed
		break;
...
*/
	...
	// RegisterItem( item );
	// Zygote - only register team items
	...
	// Zygote - make all items except team items invisible
	if ( item->giType == IT_WEAPON || item->giType == IT_AMMO ||
		 item->giType == IT_HEALTH || item->giType == IT_ARMOR ||
		 item->giType == IT_HOLDABLE || item->giType == IT_POWERUP) {
			
			ent->s.eFlags |= EF_NODRAW; // Invisible
	}

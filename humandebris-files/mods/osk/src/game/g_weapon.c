...
qboolean CheckGauntletAttack( gentity_t *ent ) {
	...
	// Zygote - Gauntlet hurts a lot more
...
void weapon_railgun_fire (gentity_t *ent) {
	...
	// Zygote Changed the damage to 1000
...
void FireWeapon( gentity_t *ent ) {
	...
	case WP_PLASMAGUN:
		...
		// Zygote - bounce plasmagun
		Bounce_Plasmagun_Fire(ent);
		break;
	case WP_RAILGUN:
		// Zygote - Telefrag or normal railgun
		if(g_oskmode.integer == 4)
		{
			weapon_telefrag_fire(ent,muzzle,forward,right,up);		// telefrag
		}else{
			weapon_railgun_fire( ent );								// killer rail			
		}
		//weapon_railgun_fire( ent );
		break;		 
	case WP_BFG:
		// Zygote - Jump and gub or delayed
		if(g_oskmode.integer == 7)
		{
			BFG_Fire1( ent );
		}else{
			BFG_Fire2( ent );
...
/*
 ****************************************************
 * Function:	Bounce_Plasmagun_Fire()	
 * Author:		Jason "Zygote" Brownlee
 *
 * This function is based on the Weapon_Plasmagun_Fire() function for 
 * the plasmagun. It has been modified so that it calls a
 * different function.
 ****************************************************
 */
void Bounce_Plasmagun_Fire (gentity_t *ent)
{
	gentity_t	*m;

	m = bounce_plasma (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
}
 ...
 * Author:		Jason "Zygote" Brownlee
 *
 * This function is based on the BFG_Fire() function for 
 * the bfg. It has been modified so that it calls a
 * different function.
 ****************************************************
 */
void BFG_Fire1 ( gentity_t *ent ) {
	gentity_t	*m;

	m = fire_jump_and_gib(ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
}
 ...
 ****************************************************
 */
void BFG_Fire2 ( gentity_t *ent ) {
	gentity_t	*m;

	m = fire_delayed(ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
}

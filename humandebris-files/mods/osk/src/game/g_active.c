...
// Zygote Start
	...
	if (client->delayswitch == qtrue) {
		if (client->delaytime < level.time) { // times up, time to DIE! :)
			switch (g_oskmode.integer) { // effect off
			case 6: // CLASSIC
				client->ps.powerups[PW_QUAD] = level.time;
				break;
			case 7: // JUMP and GIB
				client->ps.powerups[PW_FLIGHT] = level.time;
				break;
			case 8: // INVISIBLE
				client->ps.powerups[PW_INVIS] = level.time;
				break;
			case 9: // HASTE
				client->ps.powerups[PW_HASTE] = level.time;
				break;
			case 10: // BLAST!
				client->ps.powerups[PW_BATTLESUIT] = level.time;
				// where, owner, dmg, radius, ignor, mod
				G_RadiusDamage( ent->r.currentOrigin, ent, 500, 200, ent, MOD_BFG_SPLASH); // BANG!
				client->delayswitch = qfalse; // the end
				break;
			default:
				break;
			}
			//client->ps.powerups[PW_BATTLESUIT] = level.time; // effect off
			G_Damage (ent, client->delayattacker, client->delayattacker, NULL, ent->r.currentOrigin, 500, 0, MOD_BFG);
		//	return; // if you put in a return, the client lags out!
		} else { // still kicking 
			if (client->delayhitagain == qtrue) { // have been hit a second time (all but (jump n gib))
				switch (g_oskmode.integer) { // effect off
				case 6: // CLASSIC
					client->ps.powerups[PW_QUAD] = level.time;
					break;
				...
				// client->ps.powerups[PW_QUAD] = level.time; // effect off
				G_Damage (ent, client->delayattacker, client->delayattacker, NULL, ent->r.currentOrigin, 500, 0, MOD_BFG);
			}
		}
	}

	// Zygote - for effect before death for (JUMP AND GIB)
	if (client->delayeffect == qtrue) {
		client->ps.powerups[PW_FLIGHT] = level.time + 1000; 
		client->delayeffect = qfalse; // only run once otherwise it will flash
	}

	// Zygote - for effect before death (CLASSIC, INVIS, HASTE, BLAST)
	if (client->delayblasteffect == qtrue) {
	//	client->ps.powerups[PW_BATTLESUIT] = level.time + 5000; 
		switch (g_oskmode.integer) { // pick you style!!!
		case 6: // CLASSIC
			client->ps.powerups[PW_QUAD] = level.time + 5000;
			break;
		case 8: // INVISIBLE
			client->ps.powerups[PW_INVIS] = level.time + 5000;
			break;
		case 9: // HASTE
			client->ps.powerups[PW_HASTE] = level.time + 5000;
			break;
		case 10: // BLAST!
			client->ps.powerups[PW_BATTLESUIT] = level.time + 5000;
			break;
		...
		client->delayblasteffect = qfalse; // only run once otherwise it will flash
	}
// Zygote end

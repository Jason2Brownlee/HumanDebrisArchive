...
void Cmd_Kamikaze_f( gentity_t *ent ) {
	...
	// 1)=where i am, 2)=attacker(enemy), 3)=dmg, 4)=radius, 5)=who not to dmg, 6)=MOD
	G_RadiusDamage( ent->r.currentOrigin, ent, 1000, 200, NULL, MOD_GAUNTLET );
...
void Cmd_CallVote_f( gentity_t *ent ) {
	...
	if ( !Q_stricmp( arg1, "map_restart" ) ) {
		...
		trap_SendServerCommand( ent-g_entities, "print \"Invalid vote string.\n\"" );
		trap_SendServerCommand( ent-g_entities, "print \"Vote commands are: map_restart, nextmap, map <mapname>, g_gametype <n> and kick <player>.\n\"" );
		return;
	}
